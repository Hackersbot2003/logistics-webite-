...
const INP     = { width:"100%", padding:"9px 12px", border:`1px solid ${C.border}`, borderRadius:7, fontSize:14, color:C.text, outline:"none", boxSizing:"border-box", background:"#F8FAFC", fontFamily:"inherit", textTransform:"uppercase" };
...
// Force any text value to uppercase
...
function LockedBanner() {
  return (
    <div style={{ background:"#FEF9C3",border:"1px solid #FDE047",borderRadius:8,padding:"8px 14px",fontSize:12,color:"#92400E",fontWeight:600,marginBottom:10,display:"flex",alignItems:"center",gap:6 }}>
      🔒 This section is view-only for your role. Contact an admin to make changes.
    </div>
  );
}
    ...
    <div style={{ position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 0" }}>
      <div style={{ background:C.white,borderRadius:12,width:"min(95vw,780px)",margin:"20px auto",boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 24px",borderBottom:`1px solid ${C.border}` }}>
          <div>
            <h2 style={{ margin:0,fontSize:18,fontWeight:800,color:C.text }}>Vehicle Details</h2>
            <p style={{ margin:"2px 0 0",fontSize:13,color:C.muted }}>Complete information for the vehicle record</p>
          </div>
        ...
        <div style={{ padding:22,overflowY:"auto",maxHeight:"calc(100vh-140px)" }}>
          ...
          <Card><SecHdr icon="🚌" title="Vehicle Model Details" /><CardBody><G3>
            <F label="Model" value={v.model} />
            <F label="Model Info" value={v.modelInfo} />
            <F label="Model Details" value={v.modelDetails} />
            <F label="Chassis No" value={v.chassisNo} />
            <F label="Engine No" value={v.engineNo} />
            <F label="Temp Reg No" value={v.tempRegNo} />
          </G3></CardBody></Card>
          <Card><SecHdr icon="👤" title="Driver Details" /><CardBody><G3>
            <F label="Token No" value={v.tokenNo} />
            <F label="Driver Name" value={v.driverName} />
            <F label="Phone No" value={v.phoneNo} />
            <F label="Driving License No" value={v.drivingLicenseNo} />
            <F label="Incharge Name" value={v.inchargeName} />
            <F label="Current Incharge" value={v.currentIncharge} />
          </G3></CardBody></Card>
          <Card><SecHdr icon="📍" title="Tracking Info" /><CardBody><G3>
            <F label="Date" value={v.date} />
            <F label="Time" value={v.time} />
            <F label="Vehicle Location" value={v.vehicleLocation} />
            <F label="Vehicle Status" value={v.vehicleStatus} />
            <F label="Delivery Date" value={v.deliveryDate} />
            <F label="Pdi Status" value={v.pdiStatus} />
            <F label="Pdi Date" value={v.pdiDate} />
            <div>
...
function TrackingModal({ v, onClose, onSaved }) {
  const [form,setForm] = useState({
    date: v.date||TODAY_STR(), time: v.time||"",
    vehicleLocation: v.vehicleLocation||"", vehicleStatus: v.vehicleStatus||"",
    deliveryDate: v.deliveryDate||"", pdiStatus: v.pdiStatus||"",
    pdiDate: v.pdiDate||"", notes: v.notes||""
  });
  const [saving,setSaving] = useState(false);
  const set = useCallback((k,val) => setForm(p => {
    const n = {...p,[k]:val};
    if(k==="vehicleStatus" && val==="Delivered") n.deliveryDate = TODAY_STR();
    return n;
  }),[]);
  ...
  const save = async () => {
    setSaving(true);
    try { await api.put(`/vehicles/${v._id}`,form); toast.success("Tracking updated"); onSaved(); onClose(); }
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
      <div style={{ background:C.white,borderRadius:12,width:"min(95vw,780px)",boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 24px",borderBottom:`1px solid ${C.border}` }}>
          <div>
            <h2 style={{ margin:0,fontSize:17,fontWeight:800,color:C.text }}>Edit Tracking Information</h2>
            <p style={{ margin:"2px 0 0",fontSize:12,color:C.muted }}>Update tracking-related information only</p>
          </div>
          <div style={{ display:"flex",alignItems:"center",gap:14 }}>
            <span style={{ fontSize:13,color:C.muted }}>Challan No: <b style={{ color:C.blue }}>{v.challanNo}</b></span>
            <button onClick={onClose} style={{ background:"none",border:"none",fontSize:22,cursor:"pointer",color:C.faint }}>✕</button>
        ...
        <div style={{ padding:20 }}>
          <Card><SecHdr icon="📍" title="Tracking Info" /><CardBody>
            <G3>
              ...
              <Fld label="Vehicle Location"><input value={form.vehicleLocation} onChange={e=>set("vehicleLocation",UP(e.target.value))} style={INP} /></Fld>
              <div>
              ...
              <Fld label="Delivery Date"><DateInput value={form.deliveryDate} onChange={v=>set("deliveryDate",v)} /></Fld>
              <Fld label="Pdi Status">
                <select value={form.pdiStatus} onChange={e=>set("pdiStatus",e.target.value)} style={SEL}>
                  <option value="">Select PDI Status</option>
                  {PDI_STATUSES.map(s=><option key={s}>{s}</option>)}
                </select>
              </Fld>
              <Fld label="Pdi Date"><DateInput value={form.pdiDate} onChange={v=>set("pdiDate",v)} /></Fld>
              <div style={{ gridColumn:"1/-1" }}>
                <Fld label="Notes"><textarea value={form.notes} onChange={e=>set("notes",UP(e.target.value))} rows={2} style={{ ...INP,resize:"vertical" }} /></Fld>
              </div>
            </G3>
          </CardBody></Card>
        </div>
...
function FMLForm({ sheets, vehicle, onClose, onSaved }) {
  ...
  // userRestricted: editing as plain "user" → only driver & tracking sections are editable
  const userRestricted = isEdit && hasRole("user") && !hasRole("admin") && !hasRole("manager");
  ...
  useEffect(()=>{
    api.get("/logistics/fml?partner=FML").then(r=>setLogisticsItems(r.data.items)).catch(()=>{});
    api.get("/logistics/models?partner=FML").then(r=>setModelsList(r.data.items)).catch(()=>{});
  },[]);
  ...
  const modelSpecs       = modelsList
    .filter(m => m.model?.trim().toLowerCase() === selModel?.trim().toLowerCase())
  ...
  const INIT = {
    sheetName:typeSheets[0]?.sheetName||"", logisticsPartner:"FML",
    invoiceDate:TODAY_STR(), invoiceNo:"", dateOfCollection:"", dispatchDate:"", actualDispatchDate:"",
    placeOfCollection:"PITHUMPUR, M.P.", placeOfDelivery:"", otherLocationDelivery:"", overallKm:"",
    consigneeName:"", consigneeRegion:"", consigneeAddress:"",
    consignorName:"FORCE MOTOR LIMITED", consignorAddress:"PITHUMPUR, M.P.",
  ...
  const set = useCallback((k,v) => setForm(p => {
    const n = {...p,[k]:v};
    if(k==="vehicleStatus" && v==="Delivered") n.deliveryDate = TODAY_STR();
    return n;
  }),[]);
  ...
  useEffect(()=>{
    const t = setTimeout(async()=>{
      ...
      try{
        const {data} = await api.get(`/drivers/token/${tokenInput.trim().toUpperCase()}`);
        const d = data.driver; setDriverInfo(d);
        set("tokenNo",d.tokenNo); set("driverName",d.fullName||""); set("phoneNo",d.phoneNumber||"");
        set("drivingLicenseNo",d.licenseNo||""); set("inchargeName",d.inchargeName||"");
  ...
  const submit = async ()=>{
    const req = [
      ...
      "placeOfCollection","placeOfDelivery","consigneeName","overallKm","consigneeRegion","consigneeAddress",
      "consignorName","consignorAddress",
      "model","modelInfo","modelDetails","chassisNo","engineNo","tempRegNo",
      "insuranceCompany","insuranceNo","fasTagNo",
      "date","time","vehicleLocation","vehicleStatus"
    ];
    for(const key of req){
      if(!form[key]||!form[key].toString().trim()){ toast.error(`${key.replace(/([A-Z])/g,' $1')} is required`); return; }
    }
    setSaving(true);
    try{
      ...
      <div style={{ position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 0" }}>
        <div style={{ background:C.bg,width:"min(95vw,820px)",margin:"20px auto",borderRadius:12,boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
          <div style={{ background:C.white,borderRadius:"12px 12px 0 0",padding:"18px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
            <div>
              <h2 style={{ margin:0,fontSize:20,fontWeight:800,color:C.text }}>{isEdit?"Edit Vehicle Details":"Add New Vehicle"}</h2>
              <p style={{ margin:"4px 0 0",fontSize:13,color:C.muted }}>{isEdit?"Modify the vehicle details below":"Fill in the details below to add a new vehicle record"}</p>
            </div>
          ...
          <div style={{ padding:18,overflowY:"auto",maxHeight:"calc(100vh-140px)" }}>
            ...
            {/* Sheet Selector */}
            <Card><CardBody>
              {userRestricted && <LockedBanner />}
              <Fld label="Select Vehicle Sheet:" req>
                <select value={form.sheetName} onChange={e=>set("sheetName",e.target.value)} style={SEL} disabled={userRestricted}>
            ...
            {/* Vehicle & Sheet Info */}
            <Card>
              <SecHdr icon="📋" title="Vehicle & Sheet Info" />
              <CardBody>
                {userRestricted && <LockedBanner />}
            ...
            {/* Invoice & Dispatch Dates */}
            <Card>
              <SecHdr icon="📅" title="Invoice & Dispatch Dates" />
              <CardBody>
                {userRestricted && <LockedBanner />}
                <G3>
                  <Fld label="Invoice Date" req><DateInput value={form.invoiceDate} onChange={v=>set("invoiceDate",v)} readOnly={userRestricted} /></Fld>
                  <Fld label="Invoice No" req><input value={form.invoiceNo} onChange={e=>set("invoiceNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Invoice No" /></Fld>
                  <Fld label="Date Of Collection"><DateInput value={form.dateOfCollection} onChange={v=>set("dateOfCollection",v)} readOnly={userRestricted} /></Fld>
                  <Fld label="Dispatch Date"><DateInput value={form.dispatchDate} onChange={v=>set("dispatchDate",v)} readOnly={userRestricted} /></Fld>
                  <Fld label="Actual Dispatch Date"><DateInput value={form.actualDispatchDate} onChange={v=>set("actualDispatchDate",v)} readOnly={userRestricted} /></Fld>
                </G3>
              </CardBody>
            </Card>

            {/* Location & Consignee/Consignor */}
            <Card>
              <SecHdr icon="📍" title="Location & Consignee/Consignor Info" />
              <CardBody>
                {userRestricted && <LockedBanner />}
                <G3>
                  <Fld label="Place Of Collection"><input value={form.placeOfCollection} onChange={e=>set("placeOfCollection",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="Place Of Delivery" req>
                    {userRestricted
                      ? <input value={form.placeOfDelivery} readOnly style={INP_RO} />
                  ...
                  <Fld label="Consignee Name" req>
                    {userRestricted
                      ? <input value={form.consigneeName} readOnly style={INP_RO} />
                  ...
                  <Fld label="Other Location Delivery"><input value={form.otherLocationDelivery} onChange={e=>set("otherLocationDelivery",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Other Location Delivery" /></Fld>
                  ...
                  <Fld label="Consignor Name"><input value={form.consignorName} onChange={e=>set("consignorName",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="Consignor Address"><input value={form.consignorAddress} onChange={e=>set("consignorAddress",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                </G3>
              </CardBody>
            </Card>
            ...
            <Card>
              <SecHdr icon="🚌" title="Vehicle Model Details" />
              <CardBody>
                {userRestricted && <LockedBanner />}
                <G3>
                  <Fld label="Model" req>
                    {userRestricted
                      ? <input value={form.model} readOnly style={INP_RO} />
                  ...
                  <Fld label="Model Info">
                    {userRestricted
                      ? <input value={form.modelInfo} readOnly style={INP_RO} />
                  ...
                  <Fld label="Model Details">
                    {userRestricted
                      ? <input value={form.modelDetails} readOnly style={INP_RO} />
                  ...
                  <Fld label="Chassis No" req><input value={form.chassisNo} onChange={e=>set("chassisNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Chassis No" /></Fld>
                  <Fld label="Engine No" req><input value={form.engineNo} onChange={e=>set("engineNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Engine No" /></Fld>
                  <Fld label="Temp Reg No"><input value={form.tempRegNo} onChange={e=>set("tempRegNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Temp Reg No" /></Fld>
                </G3>
              </CardBody>
            </Card>
            ...
            {/* Insurance & FasTag */}
            <Card>
              <SecHdr icon="🛡️" title="Insurance & FasTag" />
              <CardBody>
                {userRestricted && <LockedBanner />}
                <G3>
                  <Fld label="Insurance Company"><input value={form.insuranceCompany} onChange={e=>set("insuranceCompany",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="Insurance No"><input value={form.insuranceNo} onChange={e=>set("insuranceNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="FasTag No"><input value={form.fasTagNo} onChange={e=>set("fasTagNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter FasTag No" /></Fld>
                </G3>
              </CardBody>
            </Card>
            ...
            <Card>
              <SecHdr icon="👤" title="Driver Details" />
              <CardBody>
                <G3>
                  <Fld label="TokenNo" req>
                    <input value={tokenInput} onChange={e=>setTokenInput(e.target.value.toUpperCase())} style={INP} placeholder="e.g. SAL01" />
                  </Fld>
                  ...
                  <Fld label="Current Incharge"><input value={form.currentIncharge} onChange={e=>set("currentIncharge",UP(e.target.value))} style={INP} placeholder="Enter Current Incharge" /></Fld>
                </G3>
              </CardBody>
            </Card>

            {/* Tracking Info — always editable */}
            <Card>
              <SecHdr icon="📍" title="Tracking Info" />
              <CardBody>
                <G3>
                  <Fld label="Date"><DateInput value={form.date} onChange={v=>set("date",v)} /></Fld>
                  <Fld label="Time"><input type="time" value={form.time} onChange={e=>set("time",e.target.value)} style={INP} /></Fld>
                  <Fld label="Vehicle Location"><input value={form.vehicleLocation} onChange={e=>set("vehicleLocation",UP(e.target.value))} style={INP} placeholder="Enter Vehicle Location" /></Fld>
                  <Fld label="Vehicle Status">
                    <select value={form.vehicleStatus} onChange={e=>set("vehicleStatus",e.target.value)} style={SEL}>
                      <option value="">Select Vehicle Status</option>
                      {VEHICLE_STATUSES.map(s=><option key={s}>{s}</option>)}
                    </select>
            ...
            {/* Billing Info — edit only */}
            {isEdit && (
              <Card>
                <SecHdr icon="🧾" title="Billing Info" />
                <CardBody>
                  {userRestricted && <LockedBanner />}
                  <div style={{ display:"flex",alignItems:"center",gap:16 }}>
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                        <input value={form.billed||""} readOnly style={{ ...INP_RO,flex:1,fontWeight:700,color:form.billed?C.green:C.faint,fontSize:15 }} placeholder="Not yet billed" />
                        {form.billed
...
function EXPForm({ sheets, vehicle, onClose, onSaved }) {
  ...
  const [form,setForm] = useState(()=>({
    ...
    placeOfCollection: vehicle?.placeOfDelivery||"",
    placeOfDelivery: vehicle?.placeOfDelivery||"", otherLocationDelivery: vehicle?.otherLocationDelivery||"",
    overallKm: vehicle?.overallKm||"", consigneeName: vehicle?.consigneeName||"",
    consigneeRegion: vehicle?.consigneeRegion||"", consigneeAddress: vehicle?.consigneeAddress||"",
    ...
    insuranceCompany: vehicle?.insuranceCompany||"BAJAJ ALLIANZ INSURANCE CO LTD",
    insuranceNo: vehicle?.insuranceNo||"OG-24-2001-9930-00000022-", fasTagNo: vehicle?.fasTagNo||"",
    tokenNo: vehicle?.tokenNo||"", driverName: vehicle?.driverName||"", phoneNo: vehicle?.phoneNo||"",
    drivingLicenseNo: vehicle?.drivingLicenseNo||"", inchargeName: vehicle?.inchargeName||"", currentIncharge: vehicle?.currentIncharge||"",
    date: vehicle?.date||TODAY_STR(), time: vehicle?.time||"", vehicleLocation: vehicle?.vehicleLocation||"",
    vehicleStatus: vehicle?.vehicleStatus||"", deliveryDate: vehicle?.deliveryDate||"",
    expecteddeliveryDate: vehicle?.expecteddeliveryDate||"", pdiStatus: vehicle?.pdiStatus||"",
    pdiDate: vehicle?.pdiDate||"", notes: vehicle?.notes||"",
  }));
  ...
  const searchFML = async ()=>{
    ...
    try{
      ...
      const fv = data.vehicle; setFmlVehicle(fv);
      ...
      set("placeOfCollection",fv.placeOfDelivery||"");
    ...
    <>
      ...
      <div style={{ position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 0" }}>
        <div style={{ background:C.bg,width:"min(95vw,820px)",margin:"20px auto",borderRadius:12,boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
          <div style={{ background:C.white,borderRadius:"12px 12px 0 0",padding:"18px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
            <div>
              <h2 style={{ margin:0,fontSize:20,fontWeight:800,color:C.text }}>{isEdit?"Edit EXP-FML Vehicle":"Add New EXP-FML Vehicle"}</h2>
              <p style={{ margin:"4px 0 0",fontSize:13,color:C.muted }}>Search FML challan to auto-fill details</p>
            </div>
          ...
          <div style={{ padding:18,overflowY:"auto",maxHeight:"calc(100vh-140px)" }}>
            ...
            {/* FML Challan Search (new only) */}
            {!isEdit && (
              <Card>
                <SecHdr icon="🔍" title="Search FML Vehicle" />
                <CardBody>
            ...
            <Card>
              ...
              <CardBody>
                {userRestricted && <LockedBanner />}
                <G3>
                  <Fld label="Invoice Date"><DateInput value={form.invoiceDate} onChange={v=>set("invoiceDate",v)} readOnly={userRestricted||(!isEdit&&!!fmlVehicle)} /></Fld>
                  <Fld label="Invoice No"><input value={form.invoiceNo} onChange={e=>set("invoiceNo",UP(e.target.value))} readOnly={userRestricted||(!isEdit&&!!fmlVehicle)} style={(userRestricted||(!isEdit&&!!fmlVehicle))?INP_RO:INP} /></Fld>
                  <Fld label="Date Of Collection"><DateInput value={form.dateOfCollection} onChange={v=>set("dateOfCollection",v)} readOnly={userRestricted} /></Fld>
                  <Fld label="Dispatch Date"><DateInput value={form.dispatchDate} onChange={v=>set("dispatchDate",v)} readOnly={userRestricted} /></Fld>
                  <Fld label="Actual Dispatch Date"><DateInput value={form.actualDispatchDate} onChange={v=>set("actualDispatchDate",v)} readOnly={userRestricted} /></Fld>
                </G3>
            ...
            <Card>
              ...
              <CardBody>
                {userRestricted && <LockedBanner />}
                <G3>
                  <Fld label="Place Of Collection (from FML)"><input value={form.placeOfCollection} onChange={e=>set("placeOfCollection",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="Place Of Delivery" req>
                    {userRestricted
                      ? <input value={form.placeOfDelivery} readOnly style={INP_RO} />
                      : <select value={form.placeOfDelivery} onChange={e=>handleDelivery(e.target.value)} style={SEL}>
                  ...
                  <Fld label="Other Location Delivery"><input value={form.otherLocationDelivery} onChange={e=>set("otherLocationDelivery",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="Overall KM"><input value={form.overallKm} readOnly style={INP_RO} /></Fld>
                  <Fld label="Consignee Region"><input value={form.consigneeRegion} readOnly style={INP_RO} /></Fld>
                  <div style={{ gridColumn:"1/-1" }}><Fld label="Consignee Address"><input value={form.consigneeAddress} readOnly style={INP_RO} /></Fld></div>
                  <Fld label="Consignor Name"><input value={form.consignorName} onChange={e=>set("consignorName",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
            ...
            {/* Model — from FML always read-only; TempRegNo editable for non-user */}
            <Card>
              <SecHdr icon="🚌" title="Vehicle Model Details" />
              <CardBody>
                {userRestricted && <LockedBanner />}
...
function OthersForm({ sheets, vehicle, onClose, onSaved }) {
  ...
  const handlePartner = (partnerName) => {
    ...
    set("placeOfCollection",""); set("placeOfDelivery","");
    set("consigneeName",""); set("consigneeRegion",""); set("consigneeAddress","");
    set("consignorName",""); set("consignorAddress","");
  };
  ...
  const handleLocation  = (loc)  => { set("placeOfDelivery",loc); set("consigneeName",""); set("consigneeRegion",""); set("consigneeAddress",""); };
    ...
    <>
      ...
      <div style={{ position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 0" }}>
        <div style={{ background:C.bg,width:"min(95vw,820px)",margin:"20px auto",borderRadius:12,boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
          <div style={{ background:C.white,borderRadius:"12px 12px 0 0",padding:"18px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
            <div>
              <h2 style={{ margin:0,fontSize:20,fontWeight:800,color:C.text }}>{isEdit?"Edit Others Vehicle":"Add New Others Vehicle"}</h2>
              <p style={{ margin:"4px 0 0",fontSize:13,color:C.muted }}>Fill in the details for an Others logistics vehicle</p>
            </div>
            <button onClick={onClose} disabled={saving} style={{ background:"none",border:"none",fontSize:22,cursor:"pointer",color:C.faint }}>✕</button>
          </div>
          ...
          <div style={{ padding:18,overflowY:"auto",maxHeight:"calc(100vh-140px)" }}>
            ...
            <Card>
              ...
              <CardBody>
                ...
                <Fld label="Logistics Partner" req>
                  {userRestricted
                    ? <input value={form.logisticsPartner} readOnly style={INP_RO} />
              ...
              <CardBody>
                ...
                <G3>
                  <Fld label="Place Of Collection">
                    {userRestricted
                      ? <input value={form.placeOfCollection} readOnly style={INP_RO} />
                  ...
                  <div style={{ gridColumn:"1/-1" }}><Fld label="Consignee Address"><input value={form.consigneeAddress} readOnly style={INP_RO} /></Fld></div>
                  <Fld label="Other Location Delivery"><input value={form.otherLocationDelivery} onChange={e=>set("otherLocationDelivery",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="Overall KM"><input value={form.overallKm} onChange={e=>set("overallKm",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Overall KM" /></Fld>
                </G3>
              </CardBody>
            </Card>
            ...
            <Card>
              ...
              <CardBody>
                {userRestricted && <LockedBanner />}
                <G3>
                  <Fld label="Model" req><input value={form.model} onChange={e=>set("model",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Model" /></Fld>
                  <Fld label="Model Info"><input value={form.modelInfo} onChange={e=>set("modelInfo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Model Info" /></Fld>
                  <Fld label="Model Details"><input value={form.modelDetails} onChange={e=>set("modelDetails",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Model Details" /></Fld>
                  <Fld label="Chassis No"><input value={form.chassisNo} onChange={e=>set("chassisNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Chassis No" /></Fld>
                  <Fld label="Engine No"><input value={form.engineNo} onChange={e=>set("engineNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Engine No" /></Fld>
                  <Fld label="Temp Reg No"><input value={form.tempRegNo} onChange={e=>set("tempRegNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} placeholder="Enter Temp Reg No" /></Fld>
                </G3>
              </CardBody>
            </Card>
            ...
            <Card>
              ...
              <CardBody>
                ...
                <G3>
                  <Fld label="Insurance Company"><input value={form.insuranceCompany} onChange={e=>set("insuranceCompany",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="Insurance No"><input value={form.insuranceNo} onChange={e=>set("insuranceNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                  <Fld label="FasTag No"><input value={form.fasTagNo} onChange={e=>set("fasTagNo",UP(e.target.value))} style={userRestricted?INP_RO:INP} readOnly={userRestricted} /></Fld>
                </G3>
              </CardBody>
            </Card>
            ...
            <Card>
              ...
              <CardBody>
                <G3>
                  ...
                  <Fld label="Incharge Name"><input value={form.inchargeName} readOnly style={INP_RO} /></Fld>
                  <Fld label="Current Incharge"><input value={form.currentIncharge} onChange={e=>set("currentIncharge",UP(e.target.value))} style={INP} /></Fld>
                </G3>
              </CardBody>
            </Card>
...
export default function Vehicles() {
  ...
  const loadSheets = useCallback(async()=>{
    try{
      ...
      if(active) setSelectedSheet(active.sheetName);
    }catch{}
  },[sheetType]);
  ...
  const fetchVehicles = useCallback(async(pg=1,q="")=>{
    ...
    try{
      const {data} = await api.get("/vehicles",{
        params:{
          sheetName: selectedSheet, page: pg, limit: 10,
          search:          q||undefined,
  ...
  const stStyle = (s) =>
    s==="Delivered"  ? {bg:"rgba(34,197,94,0.12)", color:"#16A34A"} :
    s==="In-Transit" ? {bg:"rgba(37,99,235,0.12)", color:"#2563EB"} :
    s==="Accidental" ? {bg:"rgba(239,68,68,0.12)", color:"#EF4444"} :
                       {bg:"#F1F5F9",              color:C.muted};
        ...
        <div style={{ background:C.white,border:`1px solid ${C.border}`,borderRadius:12,padding:22 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14,padding:16,background:"#F8FAFC",borderRadius:10,border:`1px solid ${C.border}` }}>
            ...
            <div>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by any field..." style={INP} />
            </div>
          </div>
          ...
          <div style={{ padding:14,background:"#F8FAFC",borderRadius:10,border:`1px solid ${C.border}`,marginBottom:16 }}>
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr auto",gap:12,alignItems:"flex-end" }}>
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:C.text,marginBottom:7 }}>Vehicle Status</div>
              ...
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:C.text,marginBottom:7 }}>PDI Status</div>
                <select value={filterPDI} onChange={e=>setFilterPDI(e.target.value)} style={SEL}>
                  <option value="">All PDI Statuses</option>
                  {PDI_STATUSES.map(s=><option key={s}>{s}</option>)}
                </select>
              </div>
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:C.text,marginBottom:7 }}>Location</div>
              ...
              <div>
                <div style={{ fontSize:13,fontWeight:600,color:C.text,marginBottom:7 }}>Model</div>
            ...
            <table style={{ width:"100%",borderCollapse:"collapse" }}>
              <thead>
                <tr style={{ background:"#F8FAFC",borderBottom:`1px solid ${C.border}` }}>
                  {["CHALLAN NO","UNIQUE ID","PLACE OF DELIVERY","DRIVER NAME","PHONE NO","CURRENT INCHARGE","VEHICLE LOCATION","VEHICLE STATUS","BILLED","ACTIONS"].map(h=>(
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={10} style={{ textAlign:"center",padding:60,color:C.faint }}>
                    <div style={{ width:32,height:32,border:`3px solid ${C.border}`,borderTopColor:C.blue,borderRadius:"50%",animation:"vspin 0.8s linear infinite",margin:"0 auto 10px" }} />
                    Loading…
                  </td></tr>
                ) : vehicles.length===0 ? (
                  <tr><td colSpan={10} style={{ textAlign:"center",padding:60,color:C.faint }}>
                    <div style={{ fontSize:36,marginBottom:8 }}>🚗</div>No vehicles found
                  </td></tr>
                ) : vehicles.map((v)=>{
                  ...
                  return (
                    <tr key={v._id}
                      ...
                      <td style={{ padding:"11px 12px" }}>
                        {v.vehicleStatus
                          ? <span style={{ background:st.bg,color:st.color,fontSize:12,fontWeight:700,padding:"3px 10px",borderRadius:20 }}>{v.vehicleStatus}</span>
                          : "—"}
                      </td>
                      <td style={{ padding:"11px 12px" }}>
                        {v.billed
          ...
          {pages>1 && (
            <div style={{ display:"flex",justifyContent:"center",alignItems:"center",gap:6,marginTop:14,paddingTop:12,borderTop:`1px solid ${C.border}` }}>
              <button onClick={()=>fetchVehicles(page-1,search)} disabled={page===1}
                style={{ padding:"7px 14px",background:page===1?"#F8FAFC":C.white,border:`1px solid ${C.border}`,borderRadius:7,cursor:page===1?"default":"pointer",color:page===1?"#CBD5E1":C.muted,fontSize:13 }}>← Prev</button>
              {Array.from({length:Math.min(pages,7)},(_,i)=>i+1).map(p=>(
                <button key={p} onClick={()=>fetchVehicles(p,search)}
                  style={{ padding:"7px 12px",background:p===page?C.blue:C.white,border:`1px solid ${p===page?C.blue:C.border}`,borderRadius:7,cursor:"pointer",color:p===page?"#fff":C.muted,fontSize:13,fontWeight:p===page?700:400 }}>{p}</button>
              ))}
              <button onClick={()=>fetchVehicles(page+1,search)} disabled={page===pages}
                style={{ padding:"7px 14px",background:page===pages?"#F8FAFC":C.white,border:`1px solid ${C.border}`,borderRadius:7,cursor:page===pages?"default":"pointer",color:page===pages?"#CBD5E1":C.muted,fontSize:13 }}>Next →</button>
            </div>
          )}
        </div>
      </div>
