...
const SEL = { ...INP, cursor:"pointer", textTransform:"uppercase" };
        ...
        <div style={{ padding:20 }}>
          <Card><SecHdr icon="📍" title="Tracking Info" /><CardBody>
            <G3>
              ...
              <div style={{ gridColumn:"1/-1" }}><Fld label="Notes"><textarea value={form.notes} onChange={e=>set("notes",UP(e.target.value))} rows={2} style={{ ...INP,resize:"vertical" }} /></Fld></div>
            </G3>
          </CardBody></Card>
        </div>
        <div style={{ padding:"14px 24px",borderTop:`1px solid ${C.border}`,display:"flex",justifyContent:"flex-end",gap:10 }}>
...
function FMLForm({ sheets, vehicle, onClose, onSaved }) {
  ...
  // userRestricted: edit mode + role is "user" → restrict non-driver/tracking sections
  ...
  const submit=async()=>{
    const req = ["invoiceDate","invoiceNo","dateOfCollection","dispatchDate","actualDispatchDate","placeOfCollection","placeOfDelivery","consigneeName","overallKm","consigneeRegion","consigneeAddress","consignorName","consignorAddress","model","modelInfo","modelDetails","chassisNo","engineNo","tempRegNo","insuranceCompany","insuranceNo","fasTagNo","date","time","vehicleLocation","vehicleStatus"];
      ...
      <div style={{ position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"flex-start",justifyContent:"center",overflowY:"auto",padding:"20px 0" }}>
        <div style={{ background:C.bg,width:"min(95vw,820px)",margin:"20px auto",borderRadius:12,boxShadow:"0 20px 60px rgba(0,0,0,0.15)" }}>
          ...
          <div style={{ padding:18,overflowY:"auto",maxHeight:"calc(100vh-140px)" }}>
            ...
            {/* Billing Info — always read-only (only shown in edit) */}
            {isEdit && (
              <Card>
...
function EXPForm({ sheets, vehicle, onClose, onSaved }) {
  ...
  const [form,setForm]=useState(()=>({
    ...
    placeOfCollection:vehicle?.placeOfDelivery||"", placeOfDelivery:vehicle?.placeOfDelivery||"", otherLocationDelivery:vehicle?.otherLocationDelivery||"",
...
function OthersForm({ sheets, vehicle, onClose, onSaved }) {
  ...
  const handleConsignee=(name)=>{ set("consigneeName",name); const c=consigneesForLoc.find(x=>x.consigneeName===name); if(c){ set("consigneeRegion",c.consigneeRegion||""); set("consigneeAddress",c.consigneeAddress||""); } };
