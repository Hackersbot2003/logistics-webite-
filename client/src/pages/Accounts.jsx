...
// ─── Financial form — FML ─────────────────────────────────────────────────────
function FMLFinancials({vehicle, fin, setF, pumps, userName, avgMileage, returnFareRef, driverWagesRef, loadingData, isUserOnly, computed}){
  const km = n(vehicle.overallKm);
  // All values come from parent-computed — single source of truth, never stale
  const { dieselQty, dieselAmt, driverWages, total, totalBorder, grandTotal, ptpAmount:ptpAmt, hpclCardDiesel:otpTotal, remaining } = computed;
...
function EXPFinancials({vehicle, fin, setF, pumps, userName, avgMileage, driverWagesRef, loadingData, isUserOnly, computed}){
  const km = n(vehicle.overallKm);
  const { dieselQty, dieselAmt, driverWages, total, totalBorder, grandTotal, ptpAmount:ptpAmt, hpclCardDiesel:otpTotal, remaining } = computed;
  const pumpTotal = (fin.petrolPumpUsage||[]).reduce((s,p)=>s+n(p.amount),0);
...
function OthersFinancials({vehicle, fin, setF, pumps, userName, computed}){
  const { dieselQty, dieselAmt, driverWages, total, totalBorder, grandTotal, ptpAmount:ptpAmt, hpclCardDiesel:otpTotal, remaining } = computed;
  const pumpTotal = (fin.petrolPumpUsage||[]).reduce((s,p)=>s+n(p.amount),0);
...
// ─── View-only Financial Display ──────────────────────────────────────────────
function ViewFinancials({vehicle, sheetType, avgMileage, returnFareRef, driverWagesRef, loadingData}){
  const VF = ({label,value,color}) => (
    <div>
      <div style={{fontSize:11,fontWeight:600,color:"#64748B",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:4}}>{label}</div>
      <div style={{...INP_RO,color:color||"#000000",fontWeight:color?700:400}}>{value||"—"}</div>
    </div>
  );

  const km = n(vehicle.overallKm);

  if(loadingData) return <div style={{padding:"30px 0",textAlign:"center"}}><Spin/><div style={{marginTop:8,fontSize:13,color:C.muted}}>Loading data…</div></div>;

  const remaining = n(vehicle.remainingBalance);

  return(<>
    <AccSection title="Financial Summary" blue>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
        <VF label="Overall KM" value={vehicle.overallKm} />
        {sheetType!=="Others" && <VF label="Avg Mileage (km/l)" value={avgMileage?avgMileage.toFixed(2):"—"} />}
        <VF label="Diesel Quantity" value={vehicle.dieselQuantity ? `${vehicle.dieselQuantity} L` : "—"} />
        <VF label="Diesel Rate" value={vehicle.dieselRate ? `₹${vehicle.dieselRate}/L` : "—"} />
        <VF label="Diesel Amount" value={vehicle.dieselAmount ? `₹${vehicle.dieselAmount}` : "—"} color={C.blue} />
        <VF label="Driver Wages" value={vehicle.driverWages ? `₹${vehicle.driverWages}` : "—"} color={C.blue} />
        <VF label="Return Fare" value={vehicle.returnFare ? `₹${vehicle.returnFare}` : "—"} />
        <VF label="Total" value={vehicle.total ? `₹${vehicle.total}` : "—"} color={C.green} />
      </div>
    </AccSection>

    <AccSection title="Expenses & Payments">
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:14}}>
        <VF label="Toll" value={vehicle.toll ? `₹${vehicle.toll}` : "—"} />
        <VF label="Border" value={vehicle.border ? `₹${vehicle.border}` : "—"} />
        <VF label="Total Border" value={vehicle.totalBorder ? `₹${vehicle.totalBorder}` : "—"} color={C.blue} />
        <VF label="4 Ltr Diesel" value={vehicle.fourLtrDiesel ? `₹${vehicle.fourLtrDiesel}` : "—"} />
        <VF label="Grand Total" value={vehicle.grandTotal ? `₹${vehicle.grandTotal}` : "—"} color={C.green} />
      </div>
      {(vehicle.taxPaymentReceipt||[]).length>0 && (
        <div>
          <div style={{fontSize:11,fontWeight:600,color:"#64748B",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>Tax Payment Receipts</div>
          {vehicle.taxPaymentReceipt.map((r,i)=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6,padding:"8px 12px",background:"#F8FAFC",borderRadius:6,border:`1px solid ${C.border}`}}>
              <span style={{fontSize:13,color:"#000"}}>{r.name||"—"}</span>
              <span style={{fontSize:13,fontWeight:600,color:C.blue}}>₹{r.amount||0}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <AccSection title="Fuel & Other Payments">
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:14}}>
        <VF label="Gate Pass" value={vehicle.gatePass ? `₹${vehicle.gatePass}` : "—"} />
        <VF label="Petty Cash" value={vehicle.pettyCash ? `₹${vehicle.pettyCash}` : "—"} />
        <VF label="PTP Amount" value={vehicle.ptpAmount ? `₹${vehicle.ptpAmount}` : "—"} color={C.blue} />
        <VF label="PTP Diesel" value={vehicle.ptpDiesel ? `₹${vehicle.ptpDiesel}` : "—"} />
        <VF label="HPCL Card Diesel" value={vehicle.hpclCardDiesel ? `₹${vehicle.hpclCardDiesel}` : "—"} color={C.blue} />
      </div>
      {(vehicle.petrolPumpUsage||[]).length>0 && (
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,fontWeight:600,color:"#64748B",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>Second Pump Diesel</div>
          {vehicle.petrolPumpUsage.map((p,i)=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6,padding:"8px 12px",background:"#F8FAFC",borderRadius:6,border:`1px solid ${C.border}`}}>
              <span style={{fontSize:13,color:"#000"}}>{p.pumpName||"—"}</span>
              <span style={{fontSize:13,fontWeight:600,color:C.blue}}>₹{p.amount||0}</span>
            </div>
          ))}
        </div>
      )}
      {(vehicle.otpProvider||[]).length>0 && (
        <div>
          <div style={{fontSize:11,fontWeight:600,color:"#64748B",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:8}}>OTP Providers (HPCL Card)</div>
          {vehicle.otpProvider.map((o,i)=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:6,padding:"8px 12px",background:"#F8FAFC",borderRadius:6,border:`1px solid ${C.border}`}}>
              <span style={{fontSize:13,color:"#000"}}>{o.name||"—"}</span>
              <span style={{fontSize:13,fontWeight:600,color:C.blue}}>₹{o.amount||0}</span>
            </div>
          ))}
        </div>
      )}
    ...
    <AccSection title="Route & Site Status">
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
        <VF label="Onroute Payment" value={vehicle.onroutePayment ? `₹${vehicle.onroutePayment}` : "—"} />
        <VF label="Onsite Receiving" value={vehicle.onsiteReceivingstatus ? `₹${vehicle.onsiteReceivingstatus}` : "—"} />
        <VF label="Misc Expenses" value={vehicle.miscellaneousExpenses ? `₹${vehicle.miscellaneousExpenses}` : "—"} />
        <VF label="Remaining Balance" value={vehicle.remainingBalance ? `₹${vehicle.remainingBalance}` : "—"} color={remaining<0?C.red:C.green} />
      </div>
      <div style={{textAlign:"center",padding:"12px 0",borderTop:`1px solid ${C.border}`,marginTop:12}}>
        <span style={{fontSize:18,fontWeight:800,color:remaining<0?C.red:C.green}}>Remaining Balance: ₹{vehicle.remainingBalance||0}</span>
      </div>
    </AccSection>
  </>);
}
...
// ─── View Modal ───────────────────────────────────────────────────────────────
function AccountsViewModal({ vehicle, sheetType, onClose, onEdit }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const [modRes, logRes] = await Promise.allSettled([
      ...
      <div style={{background:"#F3F4F6",width:"min(95vw,820px)",margin:"20px auto",borderRadius:12,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{background:"#2563EB",borderRadius:"12px 12px 0 0",padding:"16px 24px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <h2 style={{margin:0,fontSize:17,fontWeight:800,color:"#fff"}}>View Vehicle Information</h2>
            <button onClick={onClose} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:6,padding:"4px 10px",color:"#fff",cursor:"pointer",fontSize:16,lineHeight:1}}>✕</button>
          </div>
        ...
        <div style={{padding:16,overflowY:"auto",maxHeight:"calc(100vh - 200px)"}}>
          {step===1
            ...
            : <ViewFinancials vehicle={vehicle} sheetType={sheetType} avgMileage={avgMileage} returnFareRef={returnFareRef} driverWagesRef={driverWagesRef} loadingData={loadingData} />
          }
        ...
        <div style={{padding:"12px 20px",background:"#fff",borderRadius:"0 0 12px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:`1px solid ${C.border}`}}>
          <div style={{fontSize:14,fontWeight:700,color:step===2?(remaining<0?C.red:C.green):C.text}}>
            {step===2 ? `Remaining Balance: ₹${vehicle.remainingBalance||0}` : `Challan: ${vehicle.challanNo}`}
          </div>
          <div style={{display:"flex",gap:10}}>
            <button onClick={onClose} style={{...BTN("#fff",C.text,{border:`1px solid ${C.border}`})}}>Close</button>
            ...
            {step===1
              ...
              : <button onClick={onEdit} style={BTN(C.green)}>✏ Edit Accounts</button>
            }
          </div>
        </div>
      </div>
...
// ─── Shared remaining calculator (called in parent, passed as prop) ───────────
function calcRemaining(fin, sheetType, avgMileage, returnFareRef, driverWagesRef, overallKm) {
  const km = n(overallKm);
  ...
  let dieselQty, dieselAmt, driverWages, returnFare, total, grandTotal;

  if (sheetType==="FML") {
    dieselQty    = (avgMileage>0) ? int(km/avgMileage) : 0;
    dieselAmt    = int(dieselQty * n(fin.dieselRate));
    driverWages  = int(km * n(driverWagesRef));
    returnFare   = n(returnFareRef);
    total        = int(dieselAmt + driverWages + returnFare);
    grandTotal   = int(total + n(fin.toll) + totalBorder + n(fin.fourLtrDiesel));
  } else if (sheetType==="FML_EXP") {
    dieselQty    = (avgMileage>0) ? int(km/avgMileage) : 0;
    dieselAmt    = int(dieselQty * n(fin.dieselRate));
    driverWages  = int(km * n(driverWagesRef));
    returnFare   = n(fin.returnFare);
    total        = int(dieselAmt + driverWages + returnFare);
    grandTotal   = int(total + n(fin.toll) + totalBorder + n(fin.fourLtrDiesel));
  } else {
    // Others
    dieselQty    = n(fin.dieselQuantity);
    dieselAmt    = int(dieselQty * n(fin.dieselRate));
    driverWages  = n(fin.driverWages);
    returnFare   = n(fin.returnFare);
    total        = int(dieselAmt + driverWages + returnFare);
    grandTotal   = int(total + n(fin.toll) + totalBorder + n(fin.fourLtrDiesel));
  }
  ...
  return { dieselQty, dieselAmt, driverWages, returnFare, total, totalBorder, grandTotal, ptpAmount:ptpAmt, hpclCardDiesel:otpTotal, remaining };
}
...
function AccountsEditModal({ vehicle, sheetType, onClose, onSaved }) {
  ...
  const [fin,setFin] = useState({
    ...
    dieselQuantity:        vehicle.dieselQuantity||"",
    driverWages:           vehicle.driverWages||"",
  ...
  // ── Compute everything in the parent on every render ──────────────────────
  // This is the SINGLE source of truth — both the footer and the child section
  // read from this same object, so they can NEVER diverge.
  const computed = calcRemaining(fin, sheetType, avgMileage, returnFareRef, driverWagesRef, vehicle.overallKm);
        ...
        <div style={{background:"#F3F4F6",width:"min(95vw,820px)",margin:"20px auto",borderRadius:12,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
          ...
          <div style={{padding:16,overflowY:"auto",maxHeight:"calc(100vh - 200px)"}}>
            {step===1 ? <VehicleDetailsView vehicle={vehicle} /> : (
              sheetType==="FML"
                ? <FMLFinancials vehicle={vehicle} fin={fin} setF={setF} pumps={pumps} userName={userName} avgMileage={avgMileage} returnFareRef={returnFareRef} driverWagesRef={driverWagesRef} loadingData={loadingData} isUserOnly={isUserOnly} computed={computed} />
                : sheetType==="FML_EXP"
                  ? <EXPFinancials vehicle={vehicle} fin={fin} setF={setF} pumps={pumps} userName={userName} avgMileage={avgMileage} driverWagesRef={driverWagesRef} loadingData={loadingData} isUserOnly={isUserOnly} computed={computed} />
                  : <OthersFinancials vehicle={vehicle} fin={fin} setF={setF} pumps={pumps} userName={userName} computed={computed} />
            )}
            {vehicle.lastEditedBy && (
              <div style={{fontSize:12,color:C.faint,textAlign:"right",padding:"4px 0"}}>Last edited by: <b>{vehicle.lastEditedBy}</b></div>
            )}
          ...
          {/* Footer — uses same `remaining` computed in parent, never stale */}
          <div style={{padding:"12px 20px",background:"#fff",borderRadius:"0 0 12px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:`1px solid ${C.border}`}}>
            <div style={{fontSize:14,fontWeight:700,color:step===2?(remaining<0?C.red:C.green):C.text}}>
              {step===2 ? `Remaining Balance: ₹${remaining}` : `Challan: ${vehicle.challanNo}`}
            </div>
            <div style={{display:"flex",gap:10}}>
...
function AccountsTable({ sheetType }) {
  ...
  // Filters
  const [fStatus,setFStatus] = useState("Delivered");
  ...
  const [fBilled,setFBilled]     = useState("");
  const [fBilledNo,setFBilledNo] = useState("");
  const [fBillNoInput,setFBillNoInput] = useState(""); // NEW: free text bill no filter
  ...
  const hasFilters = fStatus!=="Delivered"||fPDI||fModel||fChallan||fDriver||fLocation||fOtherLoc||fChassis||fTempReg||fModelInfo||fBilled||fBillNoInput;
  const clearFilters = () => {
    setFStatus("Delivered"); setFPDI(""); setFModel(""); setFChallan(""); setFDriver("");
    setFLocation(""); setFOtherLoc(""); setFChassis(""); setFTempReg(""); setFModelInfo("");
    setFBilled(""); setFBilledNo(""); setFBillNoInput("");
    setClientPage(1);
  };
  ...
  const filtered = vehicles.filter(v => {
    ...
    // NEW: free text bill no input filter
    if (fBillNoInput && !(v.billed||"").toLowerCase().includes(fBillNoInput.toLowerCase())) return false;
    return true;
  });

  // Reset client page when filters change
  useEffect(() => { setClientPage(1); }, [fStatus,fPDI,fModel,fChallan,fDriver,fLocation,fOtherLoc,fChassis,fTempReg,fModelInfo,fBilled,fBilledNo,fBillNoInput]);

  // Client-side pagination of filtered results
  const totalFiltered = filtered.length;
  const totalClientPages = Math.max(1, Math.ceil(totalFiltered / PAGE_SIZE));
  const pagedRows = filtered.slice((clientPage-1)*PAGE_SIZE, clientPage*PAGE_SIZE);

  const stStyle = s => s==="Delivered"?{bg:"rgba(34,197,94,0.12)",color:C.green}:s==="In-Transit"?{bg:"rgba(37,99,235,0.12)",color:C.blue}:{bg:"#F1F5F9",color:"#000000"};
    ...
    <div>
      ...
      <div style={{padding:14,background:C.panel,borderRadius:10,border:`1px solid ${C.border}`,marginBottom:16}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10,alignItems:"flex-end"}}>
          ...
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Model</div>
          ...
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Location</div>
          ...
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Challan No</div>
            <input value={fChallan} onChange={e=>setFChallan(e.target.value)} placeholder="Filter…" style={INP} />
          </div>
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Driver Name</div>
            <input value={fDriver} onChange={e=>setFDriver(e.target.value)} placeholder="Filter…" style={INP} />
          </div>
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Chassis No</div>
            <input value={fChassis} onChange={e=>setFChassis(e.target.value)} placeholder="Filter…" style={INP} />
          </div>
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Temp Reg No</div>
            <input value={fTempReg} onChange={e=>setFTempReg(e.target.value)} placeholder="Filter…" style={INP} />
          </div>
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Billed Filter</div>
          ...
          {fBilled==="specific" && (
            <div>
              <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Bill No</div>
          ...
          {/* NEW: Free text Bill No search input */}
          <div>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,marginBottom:5,textTransform:"uppercase",letterSpacing:"0.05em"}}>Bill No Search</div>
            <input
              value={fBillNoInput}
              onChange={e=>setFBillNoInput(e.target.value)}
              placeholder="Type bill no…"
              style={INP}
            />
          </div>
          {hasFilters && (
            <button onClick={clearFilters} style={{...BTN("#FEF2F2",C.red,{border:`1px solid ${C.red}`,padding:"9px 14px",alignSelf:"flex-end"}),whiteSpace:"nowrap"}}>
              ✕ Clear All
            </button>
          )}
        </div>
      ...
      {/* Results count */}
      <div style={{fontSize:12,color:"#64748B",marginBottom:8,paddingLeft:2}}>
        Showing <b>{pagedRows.length}</b> of <b>{totalFiltered}</b> vehicles
        {totalFiltered !== vehicles.length && ` (filtered from ${vehicles.length} total)`}
      </div>
          ...
          <thead>
            <tr style={{background:C.panel}}>
              {["CHALLAN NO","DRIVER","DELIVERY","MODEL","STATUS","KM","DIESEL AMT","WAGES","TOTAL","GRAND TOTAL","BALANCE","BILLED","ACTIONS"].map(h=>(
                <th key={h} style={{padding:"11px 12px",textAlign:"left",fontSize:11,fontWeight:800,color:"#000000",textTransform:"uppercase",letterSpacing:"0.06em",borderBottom:`1px solid ${C.border}`,whiteSpace:"nowrap"}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              const bal = n(v.remainingBalance);
              return (
                <tr key={v._id} style={{borderBottom:`1px solid #F8FAFC`}}
                  onMouseEnter={e=>e.currentTarget.style.background=C.panel}
                  onMouseLeave={e=>e.currentTarget.style.background=C.white}>
                  <td style={{padding:"10px 12px",fontWeight:700,color:"#000000",fontSize:13}}>{v.challanNo||"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:13,color:"#000000"}}>{v.driverName||"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:12,color:"#000000",maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{v.placeOfDelivery||"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:13,color:"#000000"}}>{v.model||"—"}</td>
                  ...
                  <td style={{padding:"10px 12px",fontSize:13,color:"#000000"}}>{v.overallKm||"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:13,color:"#000000",fontFamily:"monospace"}}>{v.dieselAmount?`₹${v.dieselAmount}`:"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:13,color:"#000000",fontFamily:"monospace"}}>{v.driverWages?`₹${v.driverWages}`:"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:13,color:"#000000",fontFamily:"monospace"}}>{v.total?`₹${v.total}`:"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:13,fontWeight:700,color:C.green,fontFamily:"monospace"}}>{v.grandTotal?`₹${v.grandTotal}`:"—"}</td>
                  <td style={{padding:"10px 12px",fontSize:13,fontWeight:600,fontFamily:"monospace",color:bal<0?C.red:C.green}}>{v.remainingBalance!==undefined&&v.remainingBalance!==null&&v.remainingBalance!==""?`₹${v.remainingBalance}`:"—"}</td>
                  ...
                  <td style={{padding:"10px 12px"}}>
                    <div style={{display:"flex",gap:5}}>
                      ...
                      <button onClick={()=>setEditVehicle(v)}
                        style={BTN(hasFinancials?"#F1F5F9":C.blue,hasFinancials?"#000000":"#fff",{padding:"6px 14px",fontSize:12,border:hasFinancials?`1px solid ${C.border}`:"none"})}>
      ...
      {totalClientPages > 1 && (
        <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:6,marginTop:14,flexWrap:"wrap"}}>
          <button
            ...
            style={{padding:"7px 14px",background:clientPage===1?C.panel:C.white,border:`1px solid ${C.border}`,borderRadius:7,cursor:clientPage===1?"default":"pointer",color:clientPage===1?C.faint:"#000000",fontSize:13}}>
            ← Prev
          </button>
          {Array.from({length:Math.min(totalClientPages,7)},(_,i)=>{
            // Show pages around current page
            let p;
            if (totalClientPages<=7) p=i+1;
            else if (clientPage<=4) p=i+1;
            else if (clientPage>=totalClientPages-3) p=totalClientPages-6+i;
            else p=clientPage-3+i;
            return p;
          }).filter(p=>p>=1&&p<=totalClientPages).map(p=>(
            <button key={p} onClick={()=>setClientPage(p)}
              style={{padding:"7px 12px",background:p===clientPage?C.blue:C.white,border:`1px solid ${p===clientPage?C.blue:C.border}`,borderRadius:7,cursor:"pointer",color:p===clientPage?"#fff":"#000000",fontSize:13,fontWeight:p===clientPage?700:400}}>
              {p}
            </button>
          ))}
          <button
            ...
            style={{padding:"7px 14px",background:clientPage===totalClientPages?C.panel:C.white,border:`1px solid ${C.border}`,borderRadius:7,cursor:clientPage===totalClientPages?"default":"pointer",color:clientPage===totalClientPages?C.faint:"#000000",fontSize:13}}>
            Next →
          </button>
        </div>
      )}
      ...
      {viewVehicle && (
        <AccountsViewModal
          ...
          onEdit={()=>{setEditVehicle(viewVehicle);setViewVehicle(null);}}
        />
      )}
      ...
      {editVehicle && (
        <AccountsEditModal
          ...
          onSaved={()=>fetchVehicles(page,search)}
        />
      )}
    </div>
  );
